...
uniform ARRAY_HIGHP float ConeAngle;
uniform ARRAY_HIGHP float ConeFeather;
uniform ARRAY_HIGHP float ConeIgnoreRadius;
uniform ARRAY_HIGHP float ConeIgnoreFeather;
uniform ARRAY_HIGHP float GrainMultiplier;
uniform ARRAY_HIGHP float GrainBase;
uniform ARRAY_HIGHP float GrayscaleFactor;
uniform ARRAY_HIGHP vec3 ConeColor;
uniform ARRAY_HIGHP float ViewAngle;
...
void main()
{
 ...
 highp float ConeColor = vec3 ( ConeColor ) * ( grain * GrainMultiplier + GrainBase ) ;
 COLOR = vec4 ( mix ( color . rgb , vec3 ( ConeColor ) , mask * GrayscaleFactor ) , color . a ) ;
